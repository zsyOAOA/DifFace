...
class TrainerLatentSR(TrainerSR):
    def distance_norm2(self, x, y):
        '''
        Input:
            x: n1 x d
            x: n2 x d
        '''
        d = torch.sum(x** 2, dim=1, keepdim=True) + torch.sum(y**2, dim=1) - 2 * \
            torch.einsum('bd,dn->bn', x, rearrange(y, 'n d -> d n'))
        return d

    def get_label(self, z_gt):
        z_gt_flattened = rearrange(z_gt, 'b d h w -> (b h w) d').contiguous()
        d_gt = self.distance_norm2(z_gt_flattened, self.codebook)      # n1 x n2
        ind_gt = torch.argmin(d_gt, dim=1, keepdim=False)              # (n1,)
        return ind_gt

    def quant_distance(self, z_pred, z_gt):
        '''
        Input:
            z_pred, z_gt: n1 x d

        '''
        z_pred_flattened = rearrange(z_pred, 'b d h w -> (b h w) d').contiguous()
        d_pred = self.distance_norm2(z_pred_flattened, self.codebook)  # n1 x n2

        n1, n2 = z_pred_flattened.shape
        ind0_gt = torch.arange(0, n1, dtype=torch.int64, device=z_pred.device)      # (n1,)
        ind1_gt = self.get_label(z_gt)
        error = d_pred[ind0_gt, ind1_gt].unsqueeze(1) - d_pred                      # n1 x n2
        ...
        loss = F.relu(error, inplace=True).sum(1) / (count+1e-6)

        return loss.mean()

    def quant_label(self, z_pred, z_gt):
        '''
        Input:
            z_pred, z_gt: n1 x d
        ...
        logits = - d_pred / temperature
        target = self.get_label(z_gt)
        loss = F.cross_entropy(logits, target, reduction='mean')
        return loss

    def quant_soft(self, z_pred, z_gt):
        '''
        Input:
            z_pred, z_gt: n1 x d
        ...
        if self.num_gpus > 1:
            ...
            temperature = self.model.temperature
        logits = - d_pred / temperature
        coef = F.softmax(logits, dim=1)
        z_pred_soft = torch.einsum('bn, nd -> bd', coef, self.codebook)  # n1 x d

        ind1_gt = self.get_label(z_gt)
        target = self.codebook[ind1_gt,]                    # n1 x d

        loss = F.mse_loss(z_pred_soft, target, reduction='mean')
        return loss

    def build_model(self):
        # Diffused Estimator
        if not self.configs.model.skip_model:
            params = self.configs.model.get('params', dict)
            model = util_common.get_obj_from_str(self.configs.model.target)(**params)
            if self.configs.train.eta > 0 or self.configs.train.rho > 0:
                temperature = self.configs.model.temp_init
                if self.configs.model.temp_trainable:
                    model.temperature = nn.Parameter(torch.ones([]) * temperature)
                else:
                    model.temperature = torch.ones([]) * temperature
            if self.num_gpus > 1:
                self.model = DDP(model.cuda(), device_ids=[self.rank,])  # wrap the network
            else:
                self.model = model.cuda()
        ...
        # VQGAN Model
        params = self.configs.vqgan.get('params', dict)
        vqgan = util_common.get_obj_from_str(self.configs.vqgan.target)(**params)
        vqgan = vqgan.cuda()
        self.freeze_model(vqgan)

        encoder = util_common.get_obj_from_str(self.configs.vqgan.encoder.target)(**params)
        encoder = encoder.cuda()
        if not self.configs.vqgan.encoder_trainable:
            self.freeze_model(encoder)

        if self.rank == 0:
            self.logger.info(f'Loading VQGAN Model from {self.configs.vqgan.ckpt_path}...')
        if self.configs.vqgan.ckpt_path is not None:
            ckpt = torch.load(self.configs.vqgan.ckpt_path, map_location=f"cuda:{self.rank}")
            self.init_from_ckpt(vqgan, ckpt)
            self.init_from_ckpt(encoder, ckpt)

        # cut codebook of vqgan
        index = torch.load(
                self.configs.vqgan.codebook_index_path,
                map_location=f"cuda:{self.rank}",
                )['index']
        embed_dim = index.sum()
        codebook = vqgan.quantize.embedding.weight.data[index,]
        vqgan.quantize.embedding.weight.data = codebook
        vqgan.quantize.n_e = embed_dim
        if vqgan.quantize.remap is None:
            vqgan.quantize.re_embed = embed_dim
    ...
    def init_from_ckpt(self, model, ckpt):
        model_state = deepcopy(model.state_dict())
        for key in model_state.keys():
            assert key in ckpt, key
            model_state[key] = ckpt[key]
        model.load_state_dict(model_state)

    def print_model_info(self):
        if self.rank == 0:
            num_params = util_net.calculate_parameters(self.model) / 1000**2
            self.logger.info("Detailed architecture of the diffused estimator:")
            self.logger.info(self.model.__repr__())
            self.logger.info(f"Number of parameters in diffused estimator: {num_params:.2f}M")

            if self.configs.vqgan.encoder_trainable:
                num_params = util_net.calculate_parameters(self.encoder) / 1000**2
                self.logger.info("Detailed architecture of the encoder model:")
                self.logger.info(self.encoder.__repr__())
                self.logger.info(f"Number of parameters in Encoder Model: {num_params:.2f}M")
    ...
    def setup_optimizaton(self):
        optim_config = []
        params_model, params_temp = self.tidy_model_parameters()
        if len(params_model) > 0:
            optim_config.append({'params':params_model,})
        if self.configs.vqgan.encoder_trainable:
            optim_config.append({'params':self.encoder.parameters(), })
        if len(params_temp) > 0:
            optim_config.append({'params':params_temp, 'lr': self.configs.train.lr_temp})
    ...
    def tidy_model_parameters(self):
        params_model = []
        params_temp = []
        for key, value in self.model.named_parameters():
            if 'temperature' in key:
                params_temp.append(value)
            else:
                params_model.append(value)
        return params_model, params_temp

    def adjust_lr(self):
        if not hasattr(self, 'lr_scheduler'):
            eta_max = self.configs.train.lr
            eta_min = self.configs.train.lr_min
            ratio_iters = self.current_iters / self.configs.train.iterations
            lr_current = eta_min + 0.5 * (eta_max-eta_min) * (1 + math.cos(ratio_iters * math.pi))
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            last_batch = (jj+micro_batchsize >= current_batchsize)
            with torch.no_grad():
                z_gt = self.vqgan.encode(micro_data['gt'])
                z_gt_quant = self.vqgan.quantize_from_z(z_gt)
            z_lq = self.encoder(micro_data['lq'])
            if self.configs.model.residual:
                z_pred = self.model(z_lq) + z_lq
            else:
                z_pred = self.model(z_lq)

            losses = {}
            if last_batch or self.num_gpus <= 1:
                if self.configs.train.alpha > 0:
                    losses['mse'] = self.mse_loss(z_pred, z_gt_quant) * self.configs.train.alpha
                if self.configs.train.tau > 0:
                    losses['quant'] = self.quant_distance(z_pred, z_gt) * self.configs.train.tau
                if self.configs.train.eta > 0:
                    losses['label'] = self.quant_label(z_pred, z_gt) * self.configs.train.eta
                if self.configs.train.xi > 0:
                    losses['encodermse'] = self.mse_loss(z_lq, z_gt_quant) * self.configs.train.xi
                if self.configs.train.rho > 0:
                    losses['softmse'] = self.quant_soft(z_pred, z_gt) * self.configs.train.rho
                if self.configs.train.gamma > 0:
                    hq_pred = self.vqgan.decode(z_pred)
                    losses['lpips'] = self.lpips_loss(
                                hq_pred,
                                micro_data['gt'],
                                ).mean() * self.configs.train.gamma
            ...
            # make logging
            self.log_step_train(losses, micro_data, z_pred, z_lq, z_gt, flag=last_batch)

        self.optimizer.step()

    def log_step_train(self, losses, batch, z_pred, z_lq, z_gt, flag=False, phase='train'):
        '''
        param loss: loss value
        '''
        if self.rank == 0:
            chn = batch['lq'].shape[1]
            if self.current_iters % self.configs.train.log_freq[0] == 1:
                self.loss_mean = {key:0 for key in losses.keys()}
                self.loss_count = 0

            for key in losses.keys():
                self.loss_mean[key] += losses[key].detach().item()
            self.loss_count += 1

            if self.current_iters % self.configs.train.log_freq[0] == 0 and flag:
                for key in losses.keys():
                    self.loss_mean[key] /= self.loss_count
                log_str = 'Train:{:05d}/{:05d}, loss:{:.2e}, '.format(
                        self.current_iters // 100,
                        self.configs.train.iterations // 100,
                        self.loss_mean['loss'],
                        )
                for key, value in self.loss_mean.items():
                    if key != 'loss':
                        log_str += f'{key}:{value:.2e}, '
                exist_temp = False
                if self.num_gpus > 1 and (not self.configs.model.skip_model):
                    if hasattr(self.model.module, 'temperature'):
                        temperature = self.model.module.temperature.detach().data.item()
                        log_str += f'Temp:{temperature:.2e}, '
                        exist_temp = True
                else:
                    if hasattr(self.model, 'temperature'):
                        temperature = self.model.temperature.detach().data.item()
                        log_str += f'Temp:{temperature:.2e}, '
                        exist_temp = True
                lr = self.optimizer.param_groups[0]['lr']
                log_str += f"lr:{lr:.2e}"
                if self.configs.model.temp_trainable and exist_temp:
                    lr_temp = self.optimizer.param_groups[-1]['lr']
                    log_str += f", lr_temp:{lr_temp:.2e}"
                self.logger.info(log_str)
                # tensorboard
                self.writer.add_scalar(f'Loss-Train', self.loss_mean['loss'], self.log_step[phase])
                self.log_step[phase] += 1
            if self.current_iters % self.configs.train.log_freq[1] == 0 and flag:
                x1 = vutils.make_grid(batch['lq'], normalize=True, scale_each=True)
                self.writer.add_image("Train LQ Image", x1, self.log_step_img[phase])
                ...
                self.writer.add_image("Train HQ Image", x2, self.log_step_img[phase])
                with torch.no_grad():
                    hq_pred = self.vqgan.decode(z_pred.detach()).clamp_(-1.0, 1.0)
                    hq_recon = self.vqgan.decode(z_gt).clamp_(-1.0, 1.0)
                x3 = vutils.make_grid(hq_pred, normalize=True, scale_each=True)
                self.writer.add_image("Train Predicted HQ Image", x3, self.log_step_img[phase])
                x4 = vutils.make_grid(hq_recon, normalize=True, scale_each=True)
                self.writer.add_image("Train Reconstruct HQ Image", x4, self.log_step_img[phase])
                self.log_step_img[phase] += 1
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            total_iters = math.ceil(len(self.datasets[phase]) / self.configs.train.batch[1])
            for ii, data in enumerate(self.dataloaders[phase]):
                data = self.prepare_data(data, False)
                with torch.no_grad():
                    z_lq = self.encoder(data['lq'])
                    if self.configs.model.residual:
                        z_pred = self.model(z_lq) + z_lq
                    else:
                        z_pred = self.model(z_lq)
                    hq_pred = self.vqgan.decode(z_pred)
                    hq_pred.clamp_(-1.0, 1.0)
                    lpips = self.lpips_loss(
                            hq_pred,
                            data['gt'],
                            ).sum().item()
                psnr = util_image.batch_PSNR(
                        util_image.normalize_th(hq_pred, reverse=True),   # [0,1]
                        util_image.normalize_th(data['gt'], reverse=True),   # [0,1]
                        ycbcr=True
                        )
                ...
                if (ii+1) % self.configs.train.log_freq[2] == 0:
                    ...
                    x2 = vutils.make_grid(data['gt'], normalize=True, scale_each=True)
                    self.writer.add_image("Validation HQ Image", x2, self.log_step_img[phase])
                    x3 = vutils.make_grid(hq_pred.detach(), normalize=True, scale_each=True)
                    self.writer.add_image("Validation Recovered Image", x3, self.log_step_img[phase])
                    self.log_step_img[phase] += 1
    ...
    def build_dataloader(self, phase='val'):
        super().build_dataloader()
        if self.rank == 0:
            dataset_config = self.configs.data.get(phase, dict)
            self.datasets[phase] = create_dataset(dataset_config)
            self.dataloaders[phase] = udata.DataLoader(
                    self.datasets[phase],
                    batch_size=self.configs.train.batch[1],
                    shuffle=False,
                    drop_last=False,
    ...
    def save_ckpt(self):
        ...
        state_dict = {}
        if not self.configs.model.skip_model:
            state_dict['sr'] = self.model.state_dict()
        if self.configs.vqgan.encoder_trainable:
            state_dict['encoder'] = self.encoder.state_dict()
        torch.save({'iters_start': self.current_iters,
                    'log_step': {phase:self.log_step[phase] for phase in ['train', 'val']},
                    'log_step_img': {phase:self.log_step_img[phase] for phase in ['train', 'val']},
                    'state_dict': state_dict}, ckpt_path)
    ...
    def resume_from_ckpt(self):
        if self.configs.resume:
            ...
            if self.rank == 0:
                self.logger.info(f"=> Loaded checkpoint {ckpt_path}")
            ckpt = torch.load(ckpt_path, map_location=f"cuda:{self.rank}")
            util_net.reload_model(self.model, ckpt['state_dict']['sr'])
            if self.configs.vqgan.encoder_trainable:
                util_net.reload_model(self.encoder, ckpt['state_dict']['encoder'])
            torch.cuda.empty_cache()
