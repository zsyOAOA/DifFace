...
parser.add_argument(
        '-i',
        '--face_dir',
        default='/home/jupyter/data/FFHQ/images1024x1024',
        type=str,
        metavar='PATH',
        help="Path to save the HR face images",
        )
parser.add_argument(
        '-o',
        '--save_dir',
        default='/home/jupyter/data/FFHQ/',
        type=str,
        metavar='PATH',
        help="Path to save the resized face images",
       )
...
pch_dir = Path(args.save_dir) / f"images{pch_size}x{pch_size}"
if not pch_dir.exists():
    pch_dir.mkdir(parents=False)

transform = Aug.Compose([Aug.SmallestMaxSize(max_size=pch_size),])

# HR face path
path_hr_list = sorted([x for x in Path(args.face_dir).glob('*.'+args.ext)])
